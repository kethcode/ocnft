...
import { NFT_721E } from "./NFT_721E.sol";

/**
 * @title Composable
 * @dev An NFT format that retrieves the image metadat from remote nfts, and 
 * @dev enalbes them to build compiled into a single image offchain.
 */

contract Composable is IComposable, ERC721Enumerable, Ownable, ReentrancyGuard {
  /// ------------------------------------------------------------------------
  ...
  {
    baseURI = string(
      abi.encodePacked(_bURI, Strings.toHexString(uint160(address(this))), "/")
    );
    externalURI = _eURI;
  }
    ...
    public
  {
    ...
    for (uint256 i = 0; i < _featureHashes.length; i++) {
      selectedFeatures[_tokenId][_featureHashes[i]]
        .remoteContractAddr = address(0);
      selectedFeatures[_tokenId][_featureHashes[i]].remoteTokenId = 0;
      emit FeatureConfigured(_tokenId, _featureHashes[i]);
    }
  }
   ...
   */
  function getConfiguredFeatures(uint256 _tokenId) public view returns (string memory) {
    if (_exists(_tokenId) == false) revert invalidTokenId();
  ...
  {
    return
      interfaceId == 0x09e3ba39 || // IComposable
